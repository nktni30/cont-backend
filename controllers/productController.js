...
const createProductController = async (req, res) => {
  try {
    ...
    switch (true) {
      ...
      case !productdetails:
        return res.status(500).send({ error: "Productdeatils is Required" });
      case !brandname:
        return res.status(500).send({ error: "Brand ID is Required" });
      case !category:
        return res.status(500).send({ error: "Category ID is Required" });
      case !subcategory:
        return res.status(500).send({ error: "subcategory ID is Required" });

      // case !popular:
      //   return res.status(500).send({ error: " true or false is Required" });
